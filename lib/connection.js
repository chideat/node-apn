 ...
 Connection.prototype.serviceBuffer = function() {
    ...
    do {
        ...
        if (socket !== null) {
            ...
            // }
            this.transmitNotification(socket, this.notificationBuffer.shift());
        }
    ...
    if (data[0] == 8) {
        ...
        if (foundNotification) {
            while (temporaryCache.length) {
                var tmp = temporaryCache.shift();
                // this is really sended successfully
                this.emit('received', tmp.notification, tmp.recipient);
            }
...
Connection.prototype.transmitNotification = function(socket, notification) {
    ...
    socket.write(data, function() {
        this.socketDrained(socket, false);
    }.bind(this));
};
